...
function request_all_tasks_employee_specific(mysqli $conn, $emp_id) {
    $query = "SELECT 
        t.task_name,
        t.task_id,
        t.date,
    ...
    JOIN employee e ON t.emp_id = e.emp_id
    JOIN parties p ON t.party_id = p.party_id
    WHERE t.emp_id = ?"; // Filter by employee ID

    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $emp_id); // Assuming emp_id is an integer
    $stmt->execute();
    $result = $stmt->get_result();
    $tasks = [];
